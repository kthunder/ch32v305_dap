...
{    
    ...
    uint8_t sector = UDISK_Cur_Sec_Lba;
    ...
    /* USB上传本包数据 */
    USBHS_Endp_DataUp(DEF_UEP2, buffer+UDISK_Pack_Size * UDISK_Sec_Pack_Count,UDISK_Pack_Size, DEF_UEP_CPY_LOAD );
...
void UDISK_Down_OnePack( uint8_t *pbuf, uint16_t packlen )
{
    ...
    if( UDISK_Sec_Pack_Count == ( DEF_UDISK_SECTOR_SIZE / UDISK_Pack_Size ) )
    {
        ...
        if (UDISK_Cur_Sec_Lba<12) {
            memcpy(IFLASH_UDISK_START_ADDR + sec_start_addr, UDisk_Down_Buffer, 512);
        }else {
            // printf("sec %d\r\n", UDISK_Cur_Sec_Lba);
            // uint32_t x = 0;
            // for (int i=0; i<512; i++) {
            //     if (i<=32) {
            //         printf("0x%02X ", UDisk_Down_Buffer[i]);
            //     }
            //     x += UDisk_Down_Buffer[i];
            // }
            // printf("\r\naddr 0x%08X\r\n", 0x08002000+(UDISK_Cur_Sec_Lba-12)*DEF_FLASH_SECTOR_SIZE);
            flash_start = 1;
            flash_timer = sys_time_ms();
            IFlash_Prog_512(0x08002000+(UDISK_Cur_Sec_Lba-12)*DEF_FLASH_SECTOR_SIZE,(uint32_t*)UDisk_Down_Buffer);
        }
#endif
        /* 判断数据是否全部下传完毕 */
        if( UDISK_Transfer_DataLen == 0x00 )
